...
class StringShortener(object):
    ...
    def shorten(self, ss, spacer = ", "):
        return StringShortener._shorten(ss, [StringShortener.remove_tags, StringShortener.only_some_words, StringShortener.only_first_word, StringShortener.ellipsis, StringShortener.force], self.length, spacer)
    ...
    @staticmethod
    def test():
        assert remove_tags("[RoE] Len", None) == "Len" 
        assert remove_tags("Len", None) == "Len"
        assert remove_tags("[RoE][Side]Len", None) == "Len"
        assert remove_tags("[RoE] [Side]Len", None) == "Len"

        assert only_some_words("Len Grey the First", 10) == "Len Grey"
        assert only_some_words("Len Grey the First", 5) == "Len"
        assert only_some_words("Len Grey the First", 50) == "Len Grey the First"
        assert only_some_words("Leeeeeeeeeeeeeeeeeeen", 5) == "Leeeeeeeeeeeeeeeeeeen"

        assert only_first_word("Len Grey", None) == "Len"
        assert only_first_word("Len", None) == "Len"

        assert ellipsis("Len Grey", 10) == "Len Grey"
        assert ellipsis("Len Grey", 6) == "Len..."

        assert force("Len Grey", 10) == "Len Grey"
        assert force("Len Grey", 3) == "Len"

        print(shorten(["[DUST] Breaking Centaur/Sascha", "[DUST] Eisenfaust", "[Dust] Maxerman"], 28))
