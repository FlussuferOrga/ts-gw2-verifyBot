...
class CommanderChecker:
    def __init__(self, ts3bot, commander_group_names):
        ...
        with self.ts3bot.ts_connection_pool.item() as ts_connection:
            cgroups = list(filter(lambda g: g.get("name") in commander_group_names, ts_connection.ts3exec(lambda t: t.query("channelgrouplist").all())[0]))
        if len(cgroups) < 1:
    ...
    def execute(self):
        ...
        with self.ts3bot.ts_connection_pool.item() as ts_connection:
            acs, ts3qe = ts_connection.ts3exec(retrieve_commanders, signal_exception_handler)
                ...
                for ts_entry, db_entry in active_commanders_entries:
                    if db_entry is not None:  # or else the user with the commander group was not registered and therefore not in the DB
                        ...
                        if user.current_channel.channel_id == ts_entry.get("cid"):
                            ...
                            ac["ts_display_name"], ex1 = ts_connection.ts3exec(
                            ...
                            ac["ts_channel_name"], ex2 = ts_connection.ts3exec(lambda t, cid=ts_entry.get("cid"): t.query("channelinfo", cid=cid).first().get("channel_name"))
                            LOG.error(ex1)
                            LOG.error(ex2)
                            if ex1 or ex2:
